MODULE OBJECTIVE
	...
	CHARACTER(10), PARAMETER :: POTENTIAL_TYPE = "STEM"
        INTEGER :: NTOMS
        NTOMS = ND/3
	IF (POTENTIAL_TYPE == "GUPTA") THEN
        	CALL GUPTA(NTOMS, X, F, G, GTEST, ATOMTYPE)
   	ELSEIF(POTENTIAL_TYPE == "MORSE") THEN
		CALL MORSEGH(NTOMS, X, F, G, RHO, GTEST, HTEST)
	ELSEIF(POTENTIAL_TYPE == "LJ") THEN
		CALL LJGH(NTOMS, X, F, G,  GTEST, HTEST)
	ELSEif(POTENTIAL_TYPE == "STEM") THEN
		CALL STEM_OBJ_FUNC(ND, (ND-2)/3,  X, F, G, GTEST, HTEST)
	ELSE

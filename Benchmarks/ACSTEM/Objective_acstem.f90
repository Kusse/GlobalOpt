...
!    CHARACTER(LEN=100), PARAMETER, PRIVATE :: experimental_image_file = "img.txt" 
    ...
    REAL(KIND=dp), PARAMETER, PRIVATE  :: lowerbound_x = -image_width/2.0D0
    REAL(KIND=dp), PARAMETER, PRIVATE  :: upperbound_x = image_width/2.0D0
    REAL(KIND=dp), PARAMETER, PRIVATE  :: lowerbound_y = -image_width/2.0D0
    REAL(KIND=dp), PARAMETER, PRIVATE  :: upperbound_y = image_height/2.0D0
...
!            CALL LOADIMAGE(experimental_image_file, numrows, numcols, experimental_image)
    ...
    SUBROUTINE ACSTEM_FUNC(nd, COORDS, ENERGY, GRAD, gtest, htest)
        IMPLICIT NONE
